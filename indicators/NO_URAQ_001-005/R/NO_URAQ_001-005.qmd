...
* **PM2.5 concentration**: Raster maps with mean annual PM2.5 concentrations at national level. The PM2.5 dataset is produced using the EMEP (large scale chemistry and transport model) and uEMEP (local dispersion model) which are validated with measurements from Norwegian air quality monitoring stations by the Norwegian Meteorological Institute (@Denby2020; @Wærsted2025). It's important to note that air quality monitoring in Norway is focused on urban areas and high-traffic locations where pollution levels are a concern.
...
We downloaded the data for the years 2016 to 2024 from met.no catalogue. These data could not be loaded in the R environment directly through an API, they could only be downloaded (at the time the code was developed). We stored them on NINA's internal shared drive. To use our code more easily, we invite the users to download the data and modify the data path we provide below:
...
We have nine rasters to load, we first create a list of files before reading them and storing them in a list object.
